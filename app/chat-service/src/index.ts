...
import { ChatReceivedListener } from './events/listeners/chat-received-listener';
...
async function main() {
  ...
  await tryCatchWithRetry(
    async () => {
      await mongoose.connect(process.env.MONGO_DB_URI + '/chat');
  ...
  await tryCatchWithRetry(
    async () => {
      ...
      // Start listeners
      new ChatReceivedListener().listen();
    },
    [],
    3,
    4000
